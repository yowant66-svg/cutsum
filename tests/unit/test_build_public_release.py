...
def test_public_release_assets_are_manifested_and_checksummed(tmp_path: Path) -> None:
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "cutsum-0.1.0a1-py3-none-any.whl").write_bytes(b"wheel")
    (dist / "cutsum-0.1.0a1.tar.gz").write_bytes(b"sdist")
    ...
    sbom = json.loads((output / "cutsum-0.1.0a1.cdx.json").read_text(encoding="utf-8"))
